...
import { insertUserSchema, insertClientProfileSchema, insertTrainerProfileSchema, 
  ...
  insertTaskSchema, foods, foodCategoryEnum } from "@shared/schema";
...
import { or, eq, and, asc, desc, sql } from "drizzle-orm";
...
import nuttabRoutes from "./routes/nuttab";
import nuttabUploadRoutes from "./routes/nuttab-upload";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Food routes for nutrition data
  app.get('/api/food/search', async (req, res) => {
    try {
      ...
      const mockFoods = [
        { id: 1001, name: "Beef, ground, lean", category: "protein", servingSize: 100, servingUnit: "g", calories: 250, protein: 26.1, carbs: 0, fat: 17.2, fiber: 0 },
        { id: 1002, name: "Chicken breast, skinless", category: "protein", servingSize: 100, servingUnit: "g", calories: 165, protein: 31, carbs: 0, fat: 3.6, fiber: 0 },
        { id: 1003, name: "Salmon, Atlantic, raw", category: "protein", servingSize: 100, servingUnit: "g", calories: 206, protein: 22.1, carbs: 0, fat: 13.4, fiber: 0 },
        { id: 2001, name: "Rice, brown, cooked", category: "carbs", servingSize: 100, servingUnit: "g", calories: 112, protein: 2.6, carbs: 24, fat: 0.9, fiber: 1.8 },
        { id: 2002, name: "Sweet potato, baked", category: "carbs", servingSize: 100, servingUnit: "g", calories: 90, protein: 2, carbs: 20.7, fat: 0.2, fiber: 3.3 },
      ];
  ...
  app.post('/api/nuttab/search', async (req, res) => {
    try {
      ...
      const mockNuttabFoods = [
        ...
        { id: 2001, name: "Rice, brown, cooked", category: "carbs", servingSize: 100, servingUnit: "g", calories: 112, protein: 2.6, carbs: 24, fat: 0.9, fiber: 1.8 },
        { id: 2002, name: "Sweet potato, baked", category: "carbs", servingSize: 100, servingUnit: "g", calories: 90, protein: 2, carbs: 20.7, fat: 0.2, fiber: 3.3 },
        { id: 3001, name: "Broccoli, raw", category: "vegetable", servingSize: 100, servingUnit: "g", calories: 34, protein: 2.8, carbs: 6.6, fat: 0.4, fiber: 2.6 },
        { id: 4001, name: "Apple, with skin", category: "fruit", servingSize: 100, servingUnit: "g", calories: 52, protein: 0.3, carbs: 13.8, fat: 0.2, fiber: 2.4 },
        { id: 6001, name: "Almonds, raw", category: "nuts", servingSize: 28, servingUnit: "g", calories: 164, protein: 6, carbs: 6, fat: 14, fiber: 3.5 }
      ];
  ...
  app.post('/api/nuttab/import', async (req, res) => {
    try {
      ...
      // In a real implementation, this would insert the foods into the database
      // For now, we'll just return the foods as if they were successfully imported
      
      res.status(201).json({ 
        message: `Successfully imported ${foodsToImport.length} foods`, 
        foods: foodsToImport 
      });
    } catch (error) {
    ...
    try {
      ...
      function mapToFoodCategory(nuttabCategory: string): string {
        ...
        for (const [key, value] of Object.entries(categoryMap)) {
          if (nuttabCategory && nuttabCategory.toLowerCase().includes(key.toLowerCase())) {
            return value;
          }
        }
      ...
      // Try to batch insert all foods
      try {
        const result = await db.insert(foods).values(processedFoods).returning();
        insertedFoods.push(...result);
        console.log(`Successfully inserted ${result.length} foods in batch`);
      } catch (batchError) {
        console.error('Batch insert failed, trying individual inserts:', batchError);
        
        // If batch insert fails, try inserting foods one by one
      ...
      res.status(201).json({
        message: `Successfully imported ${insertedFoods.length} foods from NUTTAB Excel file`,
        ...
        // Return the first few foods as a sample
        sampleFoods: insertedFoods.slice(0, 5)
      });
