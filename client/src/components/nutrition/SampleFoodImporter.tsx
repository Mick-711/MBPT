...
const SampleFoodImporter: React.FC = () => {
  ...
  const [sampleSize, setSampleSize] = useState(25); // Default sample size
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Generate preview with first few data rows
          ...
          // Start with the row after the header row and only take a sample
          const allDataRows = jsonData.slice(headerRowIndex + 1) as any[];
          
          // Take a random sample instead of just the first n rows
          let sampleRows: any[] = [];
          if (sampleSize >= allDataRows.length) {
            sampleRows = allDataRows; // Use all if sample size is larger
          } else {
            // Take every nth row to get a representative sample
            const step = Math.floor(allDataRows.length / sampleSize);
            for (let i = 0; i < allDataRows.length; i += step) {
              if (sampleRows.length < sampleSize) {
                sampleRows.push(allDataRows[i]);
              } else {
                break;
              }
            }
          ...
          const processedFoods = sampleRows
            ...
            .map(row => {
              ...
              return {
                ...
                fiber: parseFloat(row[mapping.fiber]) || 0,
          ...
          // Process in very small batches (2 foods at a time) to reduce payload size
          const BATCH_SIZE = 2;
          const chunks = [];
          for (let i = 0; i < processedFoods.length; i += BATCH_SIZE) {
            chunks.push(processedFoods.slice(i, i + BATCH_SIZE));
          }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Food Sample Importer
          </CardTitle>
          <CardDescription>
            Import a sample of foods from the Australian Food Composition Database
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Select the NUTTAB Excel file. Only a sample of foods will be imported.
              </p>
            </div>
            
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Sample Size: {sampleSize} foods</span>
              </div>
              <Slider
              ...
              <p className="text-xs text-muted-foreground">
                Choose how many foods to import (smaller batches are more reliable)
              </p>
            </div>
            ...
            {file && (
              ...
              >
                ...
                {isLoading ? 'Processing...' : 'Process Sample'}
              </Button>
            )}
...
export default SampleFoodImporter;